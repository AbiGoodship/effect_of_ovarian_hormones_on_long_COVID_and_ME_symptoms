...
if(file.exists("dim_001_userinfo__user-000000000000.csv"))
{
  ...
  Full_app_data_clean <- Full_app_data %>% filter(user_id_pk %in% common_users)
  Full_survey_data_clean <- Full_survey_data %>% filter(user_id_pk %in% common_users)
  Full_user_info_clean <- Full_user_info %>% filter(user_id_pk %in% common_users)
  
  # Remove any duplicate user rows in user info, keeping the first occurrence
  Full_user_info_clean <- Full_user_info_clean %>%
    distinct(user_id_pk, .keep_all = TRUE)
  
  n_distinct(Full_app_data_clean$user_id_pk)
  ...
  Cleaned_user_info <- Full_user_info_clean[!(Full_user_info_clean$user_id_pk %in% Users_to_exclude), ]
  ...
  # Extract year of survey completion, if multiple survey entries, picks the first occurrence
  User_dates <- Cleaned_survey_data %>%
    mutate(submission_timestamp = as.Date(submission_timestamp)) %>%
    group_by(user_id_pk) %>%
    summarise(
      submission_timestamp = first(submission_timestamp),  
  ...
  #'*Simplify and categorise demographic info so all in user info*
  ...
  # Age category and disease type simple
  Cleaned_user_info <- Cleaned_user_info %>%
    mutate(
  ...
  # Gravidity
  Gravidity_summarised <- Cleaned_survey_data %>%
  ...
  Contraception <- Cleaned_survey_data %>%
    filter(question_id_pk == 1) %>% 
    select(user_id_pk, answer) %>%
    mutate(Contraception_type = case_when(
      answer %in% c("Combined pill", "Contraceptive patch") ~ "Estrogen & Progestin",
  ...
  Cleaned_user_info <- Cleaned_user_info %>%
    left_join(Contraception, by = "user_id_pk")
  ...
  Symptom_lookup <- Symptom_info %>%
    inner_join(Symptom_classification_info, by = "health_variable_classification_id") %>%
    select(-user_id) %>%
    filter(is_published == "true")
  
  Cleaned_app_data <- Cleaned_app_data %>%
    inner_join(Symptom_lookup, by = "health_variable_id_pk")
  
  Cleaned_app_data$observation_date_pk <- as.Date(Cleaned_app_data$observation_date_pk)
  
  #'*Set aside 1932 users to look at demographics later*
  
  Cleaned_user_info_1932 <- Cleaned_user_info 
  Cleaned_app_data_1932 <- Cleaned_app_data 
  
  n_distinct(Cleaned_user_info_1932$user_id_pk)
  n_distinct(Cleaned_app_data_1932$user_id_pk)
  ...
  # Remove those who never report a period
  ...
  #'*Set aside 1765 users to look at demographics later*

  Cleaned_app_data_1765 <- Cleaned_app_data %>%
    filter(user_id_pk %in% Users_with_periods)
  
  Cleaned_user_info_1765 <- Cleaned_user_info %>%
    filter(user_id_pk %in% Users_with_periods)
  
  n_distinct(Cleaned_app_data_1765$user_id_pk)
  n_distinct(Cleaned_user_info_1765$user_id_pk)
  
  #'*Add in missing dates for period data*
  ...
  # Replace missing values for added in dates  with '2'
  ...
  # Find number of cycles where there are not more than two consecutive missing days
  Number_cycles_missing_less_2_days <- Cycles_missing_less_2_days %>%
    group_by(user_id_pk, period_group) %>%
    summarise(num_rows = n())
  
  #'*Set aside 1109 users to look at demographics later*

  # Join menstrual data & symptom data 
  
  # Add missing dates to app data
  Complete_app_data_1109 <- Cleaned_app_data_1765 %>%
    filter(!(observation_date_pk < "2022-09-07")) %>%
    complete(user_id_pk, observation_date_pk) 
  
  # Join menstrual data & symptom data 
  Cleaned_app_data_1109 <- Complete_app_data_1109 %>%
    left_join(
      Cycles_missing_less_2_days,
      by = c("user_id_pk", "observation_date_pk"),
      suffix = c("", ".x")) %>%
    select(
      -ends_with(".x")
    )
  
  n_distinct(Cleaned_app_data_1109$user_id_pk)
  n_distinct(Cleaned_app_data_1109$cycle_id)
  
  # Filter to get data for the 1109 users
  user_ids_1109 <- unique(Cycles_missing_less_2_days$user_id_pk)
  
  Cleaned_user_info_1109 <- Cleaned_user_info %>%
    filter(user_id_pk %in% user_ids_1109)
  
  n_distinct(Cycles_missing_less_2_days$user_id_pk)
  n_distinct(Cleaned_app_data_1109$user_id_pk)
  n_distinct(Cleaned_user_info_1109$user_id_pk)
  ...
  # Make sure values are factors and dates are dates
  ...
  #'*Assign menstrual cycle phase*
  
  # For each cycle, identify period start and end dates and propagate them to all observations within the cycle
  ...
  # Classify each observation into menstrual phases based on period dates and proximity to the next menstrual period
  ...
  # Add missing dates to app data for full dataset
  ...
  Menstrual_and_all_symptom_data_all_cycles_within_range <- Complete_app_data %>%
    ...
    select(-health_variable_id_pk.x, -observation_value.x, -quality.x, -observation_descriptor.x,
           -rating_scale_value_id.x, -row_id.x, -updated_at.x)
  ...
  # Find and remove any retrospective symptom reporting (> 2 days)
